# -*- coding: utf-8 -*-
"""
Created on Mon Feb 22 12:06:19 2021
...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 22 11:48:53 2021
...
eps_data = all_sites_eps(seqVar, 'essential')
#%% Visualizations ##################
...
def single_site_graphic(X, Hexp):
    ...
    return fig, ax
#%% Figure 1
def create_figure_8(matrix, X, Hexp):
    fig, (ax1, ax2) = plt.subplots(1,2, figsize = (13,6.5))
    fig.suptitle('Figure 8: S1B Family Essential Landscape', size=24, weight='bold')
    #########################
    value = 0.0
    masked_array = np.ma.masked_where(matrix == value, matrix)
    cmap_avg = matplotlib.cm.winter
    cmap_avg.set_bad(color='black')
    #########################
    ax1.imshow(masked_array, cmap=cmap_avg)
    ...
    #########################
    rect29a = plt.Rectangle((1.5, 1.5), 1, 20, fill=False, color='tab:red', linewidth=2.0)
    rect29b = plt.Rectangle((-0.5, 1.5), 3, 1, fill=False, color='tab:red', linewidth=2.0)
    ax1.add_patch(rect29a)
    ax1.add_patch(rect29b)
    #########################
    rect38a = plt.Rectangle((10.5, 10.5), 1, 11, fill=False, color='tab:orange', linewidth=2.0)
    rect38b = plt.Rectangle((-0.5, 10.5), 12, 1, fill=False, color='tab:orange', linewidth=2.0)
    ax1.add_patch(rect38a)
    ax1.add_patch(rect38b)
    #########################
...
create_figure_8(eps_data[0], X, H)
